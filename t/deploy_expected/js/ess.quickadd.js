...
Event.observe(document,'dom:loaded',function(e){
	...
	if(mainProductImage){
		mainProductImage.observeProductViewChange=function(product){
			...
			var loader=$(loaderId);
			loader.setStyle({opacity:.5,top:((parseInt(this.getStyle('height'))/2)-(parseInt(loader.getStyle('height'))/2))+'px'});
			loader.show();
	...
	var shopWithConfidenceLink=$$('.policy-link')[0];
	if(shopWithConfidenceLink)shopWithConfidenceLink.observe('click',function(e){e.stop();window.parent.location=this.readAttribute('href');window.parent.modal.close();});
});