...
Store.DhtmlComboAttribute.addMethods({
	...
	this.element.update(this.pars.selectText)
	this.repopulateData(
	    this.getAvailableAttributeValues().collect(function(rec){
                return {
                    'label': (this.getListItemLabel(rec) || this.getListItemValue(rec)),
                    'value': this.getListItemValue(rec)
                }
            }.bind(this))
        )

        if(this.getAvailableAttributeValues().length == 1){
                this.setCurrentValue(this.getAvailableAttributeValues()[0])
        }
...
Store.ES = Class.create(Df.Element, {
    ...
    fireAnalytics: function(){
        ...
        var memo = {
                ...
                mfgStyleCode: ess.productJSON.skus[0].jdaStyle,
                type: 'STANDARD',
                available: true
        }
...
Store.Cart = Class.create(Df.Base, {
    ...
    getCart: function(action){
        ShoppingCartServiceWrapper.getNumberOfItems(function(response) {
            this._getCartItems(response)
            this.totalItemsInCart = response
            if(response == 0 ){
                $(document.body).fire(':cartEmpty')
            }
        }.bind(this))

        ShoppingCartServiceWrapper.getTotal(function(response) {
            this._cartTotal = response
        }.bind(this))
        return this;
    },
    ...
    _addToCart: function(e){
        ...
        e.memo.products.each(function(v){
            ...
            ShoppingCartServiceWrapper.add(requestType, addRequest, function(response) {
              ...
              //Fire analytics for first product added to cart from empty cart.
              ...
              if(response[0].toString() == "OK"){
				  if('function'==typeof(e.memo.onOpen))e.memo.onOpen();
				  document.observe('click', function(e) {
				  	var mcChildren = $('minicart-container').descendants();
				  	var clickSpot = Event.element(e);
				  	var close = false;
				  	
				  	mcChildren.each( function(el) {
				  		if ( clickSpot == el ) {
				  			close = false;
				  			throw $break;
				  		} else {
