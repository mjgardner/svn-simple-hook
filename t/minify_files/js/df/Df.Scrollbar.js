...
Df.Scrollbar = Class.create(Df.Ui, {
	...
	_setup: function($super){
		...
		this.__bodyObservers = this._bodyObservers.bind(this);

		this.adjustToContent()
	},

	_bodyObservers: function(e){
		Event.stop(e)
		this.yClickStatusNext = false;
		this.xClickStatusNext = false;
		this.yClickStatusPrev = false;
		this.xClickStatusPrev = false;
		Event.stopObserving(document.body, 'mouseup', this.__bodyObservers)
	},
	...
	*/
	...
	xMakeScrollDecision: function(event){
		...
		Event.observe(document.body, 'mouseup', this.__bodyObservers);
...
Df.PagingScrollbar = Class.create(Df.Scrollbar, {
	...
	scrollAction: function(e){
		var t = setTimeout(this.changePageLogic.bind(this, this.element.scrollTop, this.element.scrollLeft),this.pars.delay)
	},
	...
	changePageLogic: function(oldScrollTop, oldScrollLeft){
		if(this.element.scrollTop == oldScrollTop && this.element.scrollLeft == oldScrollLeft){
			...
			var rows = $A($R((rowTop - this.pars.rowsOverlap).toRange(1, this.pars.rows),(rowTop - 1 + this.rowsInView + this.pars.rowsOverlap).toRange(1, this.pars.rows)));

			var cols = $A($R((rowLeft - this.pars.colsOverlap).toRange(1, this.pars.cols),(rowLeft - 1 + this.colsInView + this.pars.colsOverlap).toRange(1, this.pars.cols)));
			var newCellsToShow = [];
