...
var storeLocator = {
	...
	pars: {
		operation:'coSearch',
		lat:54.533833, // Latitude for initial map (PEAC)
		lon:-2.504883,  // Longitude for initial map (PEAC)
		numResults:100,
		mnlt:false,
		mxlt:false,
		mnln:false,
	...
	buildAddress: function(){
		...
		if((city && state)|| zip){
			...
			storeLocator.resetPage();
			storeLocator.msg({dataString:'Please enter a city or a postcode.'});
	...
	getLocation: function(){
		...
		geocoder.getLatLng(storeLocator.vars.address,function(point){
			...
			if(point == null){
				storeLocator.resetPage()
				storeLocator.msg({dataString:'Search not recognized'})
				$('ResultList').update('<div class="paddMsg"><p class="fB">Sorry, we could not recognize your search.</p><p>While we have a vast network of stores, no stores were found using your current selection.</p><ul>Please refine your search by:<li>Altering your search parameters </li><li>Expanding your zoom level</li></ul></div>')
	...
	showList: function (originalRequest){
		...
		if(obj.COUNT==0){
			...
			$('ResultList').update('<p>Sorry, no locations were found in this area. <strong style="color:#d81027">Tip: Try zooming out further</strong></p><p>While we have a vast network of stores, no stores were found using your current selection.</p><p><strong>Please refine your search by:</p></strong><ul><li>Altering your search parameters</li><li>Expanding your zoom level</li></ul>')
			return
		}
		...
		//update zoom button state - especially required for first time search
		...
		obj.RESULTS.each(function(v,i){
			if(v.store){
				...
				if (v.store.mallPlazaName != null) {
					var a = storeInfo.e('div').addClassName("storeName").update("<div>" + v.store.locationName + "</div>" + v.store.mallPlazaName)
				}
				else {
					var a = storeInfo.e('div').addClassName("storeName2").update(v.store.locationName)
				}
				...
				// Store code
				storeInfo.e('div').addClassName("locationCode").update("store #"+v.store.locationCode)

				//location code
				//$E('div',rec,{className:"locationCode",innerHTML:'#'+v.store.locationCode})
				//address
				storeInfo.e('div').addClassName("address").update("<span title='"+v.store.address1+"'>"+v.store.address1+"</span>")
				if(v.store.address2){
					storeInfo.e('div').addClassName("address").update("<span title='"+v.store.address2+"'>"+v.store.address2+"</span>")
				}
				//storeInfo.e('div').addClassName("address").update(v.store.city+' '+v.store.stateCode+ ' ' +v.store.postalCode)
				storeInfo.e('div').addClassName("address").update(v.store.city+' '+v.store.postalCode); // modified address format (PEAC)
				...
				if(storeLocator.vars.miles){
					storeInfo.e('div').addClassName("miles").update(v['distance'].round(1) + ' Miles')
				}
				...
				//store hours link
				var a = storeInfo.e('a').addClassName("storeHours").update(' | Show On Map')
...
storeLocator.map = {
	...
	initialSetup: function(map){
		map.setCenter(new GLatLng(storeLocator.pars.lat, storeLocator.pars.lon), 6);
		if (!storeLocator.vars.storeAvail)
		{
			$('ResultHeader').update('<h2 class="welcome">Finding the store<br/> nearest to you is easy.</h2>')
			$('ResultList').update('<p>Enter your <strong>address</strong>, <strong>city</strong>, or <strong>post code</strong> in the search field above to find the Peacocks near you.</p>')
		}
		
		// PEAC Specific - required to addMapControlls at initial load
		storeLocator.map.addMapControls(map);
		storeLocator.vars.zoom = 13; // need to initialize again as addMapControls modifies it with 7 from line#670
	...
	addMapControls: function(map){
		...
		obj.e('div','bottom',{className:'control panUp',
			...
			storeLocator.vars.miles = true; // PEAC
		}});
		//down
		...
		// Move tooltip to main container (PEAC specific)
		var ptt = $('ptooltip');
		$('main-container').e('div', 'bottom', {id: 'ptooltip', className:'ptooltip left'}).update(ptt.innerHTML);
		ptt.remove();
		...
		$('fldLocation').value = 'Enter address, city or post code';
		$('fldLocation').observe('focus', function(e){
            if (this.value == 'Enter address, city or post code') {
                this.value='';
                this.style.color = '#000000';
            }
		});
		$('fldLocation').observe('blur', function(e){
            if (this.value == '') {
                this.value='Enter address, city or post code';
	...
	addEvents: function(map){
		GEvent.addListener(map, "dragstart", function() {
			storeLocator.vars.miles = true; // show miles when dragging map (PEAC)
		});
	...
	markerToPixel: function(marker){
		...
		if(par){
			...
			var pos = Position.cumulativeOffset($('store-locator')) // changed id (PEAC)
...
storeLocator.map.bubble = {
	...
	store: function(i,v){
		...
		if(v.store.storeHours){
			...
			//store Name
			content2.e('div').addClassName("storeName").update(v.store.locationName)
			//location code
			//content2.e('div').addClassName("locationCode").update('store #'+v.store.locationCode)
		...
		//store Name
		content.e('div').addClassName("storeName ").update(v.store.locationName)

		//location code
		content.e('div').addClassName("locationCode").update('store #'+v.store.locationCode); // PEAC
		...
		content.e('div').addClassName("address").update(v.store.city+ ' ' +v.store.postalCode); // removed state (PEAC)
		...
		//directions
		var a = content.e('a','bottom',{href:storeLocator.val.directions(v), className:"directions", target:"_blank", innerHTML:'Directions'})
	...
	show: function(i,v,rec,marker,pan){
		...
		var left = pixels.x - 97; // changed x offset (PEAC)
		var top = pixels.y - 145; // changed y offset (PEAC)

		//show bubble
		var bubble = storeLocator.map.bubble.store(i,v)
		bubble.style.left = left + 'px'
		bubble.style.top = top + 'px'
		$('store-locator').insert(bubble); // changed id (PEAC)
		...
		$(document).fire("sl:store-locator-store-select", { locationCode : v.store.locationCode } )
	}
}
...
storeLocator.list = {
	...
	updateStoreNumber: function(obj){
		...
		var html = '<div class="fB2">' + obj.COUNT + '</div><div class="fB3">There is one store in your area</div>'
		if(obj.COUNT > 1){
			html = '<div class="fB2">' + obj.COUNT + '</div><div class="fB3">There are ' + obj.COUNT + '<br/>stores in your area</div>'
		}
...
storeLocator.val = {
	...
	phone: function(string){
		...
		if(string && string.length == 10){
			return '<span>Phone: </span>('+string.substring(0,3)+') '+string.substring(3,6)+'-'+string.substring(6,10)
		}else{
			return '<span>Phone: </span>'+string
		}
	},

	directions: function(v){
		if(storeLocator.vars.address != " "){
			return 'http://maps.google.co.uk/maps?saddr='+storeLocator.pars.lat+','+storeLocator.pars.lon+'('+encodeURI(storeLocator.vars.address)+')&daddr='+v.store.latitude+','+v.store.longitude+'('+encodeURI(v.store.address1+' '+v.store.city+', '+v.store.postalCode)+')'; // removed state (PEAC)
		}else{
			return 'http://maps.google.co.uk/maps?q='+v.store.latitude+','+v.store.longitude+'('+encodeURI(v.store.address1+' '+v.store.city+', '+v.store.postalCode)+')'; // removed state (PEAC)
		}
	}
}
